...
const TagsInputField = ({ name, label, placeholder, rules }: TagsInputFieldProps) => {
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap gap-2 rounded-[14px] py-2 px-3 border border-[#D1DAEC]">
        {field.value?.map((tag: string, idx: number) => (
          <div key={idx} className="flex items-center gap-1 bg-primary text-white rounded-lg px-2 py-1 text-sm">
            {tag}
            <X
              size={14}
