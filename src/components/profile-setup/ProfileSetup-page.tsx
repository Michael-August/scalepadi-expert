...
const ProfileSetUp = () => {
    ...
    useEffect(() => {
        if (stepParam) {
            const stepIndex = steps.findIndex(step => step.toLowerCase().replace(/\s+/g, '-') === stepParam.toLowerCase());
            if (stepIndex !== -1) {
    ...
    const onSubmitFinal = (data: any) => {
        ...
        let completionRate = user?.regPercentage && user.regPercentage >= stepRate
            ? user.regPercentage
            : stepRate;
        ...
        if (currentStep === 1) {
            ...
            completeProfileSetup({ ...payload, regPercentage: completionRate }, {
                onSuccess: () => {
            ...
            Object.keys(data).forEach((key) => {
                if (key === "identification") {
                    if (data.identification?.[0]) {
                        formData.append("idImage", data.identification[0]);
                    }
                } else {
    ...
    useEffect(() => {
        ...
        methods.reset({
            ...
            role: Array.isArray(storedUser?.role) ? storedUser.role.filter((r: string) => r !== "undefined") : [],
            preferredIndustry: Array.isArray(storedUser?.preferredIndustry)
            ? storedUser.preferredIndustry.filter((ind: string) => ind !== "undefined")
            : [],
    ...
    return (
        <div className="flex flex-col gap-10 pb-20">
            <div className="heading flex items-center gap-[9px] w-full px-14 py-4 bg-[#F8F8F8]">
                {steps.map((step, idx) => (
                    <div key={idx} onClick={onNext} className="flex items-center gap-2 cursor-pointer">
                        <div
