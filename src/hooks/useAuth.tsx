    ...
    const { mutate: setexpertDetails, isPending } = useMutation({
        mutationFn: async (data: any) => {
            try {
                ...
                if (res.data?.status === false) {
                    throw new Error(res.data?.message || "An error occurred while setting expert details");
                }
                return res.data;
            } catch (error: any) { 
                throw new Error(error) || new Error("An error occurred while setting expert details");
            }
        }
    })
    ...
    const { mutate: completeProfileSetup, isPending } = useMutation({
        mutationFn: async (data: any) => {
            try {
                ...
                if (res.data?.status === false) {
                    throw new Error(res.data?.message || "An error occurred during profile update");
                }
                return res.data;
            } catch (error: any) {
                // If backend sent a message, preserve it
