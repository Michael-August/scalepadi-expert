		...
		queryFn: async () => {
			try {
				const response = await axiosClient.get(
					`/projects/account/expert?${new URLSearchParams(
						params
					).toString()}`
				);
				...
				if (error instanceof AxiosError) {
					...
					toast.error(
						"An unexpected error occured while fetching projects"
					);
				}
		...
		queryFn: async () => {
			try {
				...
				if (error instanceof AxiosError) {
					...
					toast.error(
						"An unexpected error occured while fetching project"
					);
				}
...
export const useAcceptDeclineHire = () => {
	...
	const { mutate: acceptOrDeclineHire, isPending } = useMutation({
		mutationFn: async (data: {
			expertStatus: "accepted" | "declined";
			hireId: string;
		}) => {
			try {
				const response = await axiosClient.put(
					`/hire/expert/${data?.hireId}`,
					{ expertStatus: data.expertStatus }
				);
				if (response.data?.status === false) {
					throw new Error(
						response.data?.message ||
							`Failed to ${data.expertStatus} match`
					);
				}
				return response.data;
			} catch (error: any) {
				if (error instanceof AxiosError) {
					...
					toast.error(
						`An unexpected error occured while trying to ${data.expertStatus} match`
					);
				}
	...
	const { mutate: submitTask, isPending } = useMutation({
		mutationFn: async (data: any) => {
			try {
				...
				if (response.data?.status === false) {
					throw new Error(
						response.data?.message || `Failed to submit task`
					);
				}
				return response.data;
			} catch (error: any) {
				if (error instanceof AxiosError) {
					...
					toast.error(
						`An unexpected error occured while trying to submit task`
					);
				}
		...
		mutationFn: async (data: {
			...
			try {
				...
				const backendMessage =
					error?.response?.data?.message ||
					error?.message ||
					"An error occurred during inquiry";
