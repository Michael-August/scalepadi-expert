  ...
  const handleAcceptDecline = (
    ...
    acceptOrDecline(
      { response: action, projectId },
      {
        ...
        onError: () => {
          toast.error(
            `Error ${action === "accepted" ? "accepting" : "declining"} project`
          );
        },
      }
    );
  ...
  const handleAcceptDeclineHire = (
    ...
    acceptOrDeclineHire(
      { expertStatus: action, hireId },
      {
        ...
        onError: () => {
          toast.error(
            `Error ${action === "accepted" ? "accepting" : "declining"} hire`
          );
        },
      }
    );
      ...
      <div className="flex w-full flex-col gap-6">
        ...
        <div className="w-full lg:w-[895px] pb-10">
                  ...
                  >
                    <div className="top border-b border-primary-border pb-3 w-full flex items-center gap-3">
                      <div className="bg-[#D1F7FF] flex items-center justify-center p-[5.84px] text-[#1A1A1A] text-xs h-[54px] w-[54px] rounded-[11.68px]">
                        {project?.businessId?.title?.trim().split(" ")[0] ||
                          "B"}
                      </div>
                    ...
                    {project?.experts?.some(
                      (expert: any) =>
                        expert.id.id === user.id && expert.status === "pending"
                    ) && (
                      <div className="flex gap-4">
                        <Button
                          disabled={isPending}
                          onClick={() =>
                            handleAcceptDecline("accepted", project.id)
                          }
          ...
          {activeTab === "business" && (
            <div className="rounded-3xl gap-3 w-full border border-[#D1DAEC80] hide-scrollbar p-3 h-[862px] overflow-y-scroll">
              <div className="h-full flex flex-col gap-4 w-full bg-[#FBFCFC] rounded-3xl p-3 hide-scrollbar overflow-y-scroll">
                {hires?.data?.data?.length === 0 && !isLoadingHires && (
                  <div className="flex flex-col gap-2 items-center justify-center h-full">
                    <span className="text-sm text-[#878A93]">
                      No opportunities available at the moment
                    </span>
                ...
                {hires?.data?.data?.map((hire: any) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <span className="text-[#727374] text-sm">
                        {hire?.budget.toLocaleString() || "N/A"}
                      </span>
                    </div>
                    <div className="flex flex-col gap-2">
                    ...
                    {hire?.expertStatus === "awaiting-response" && (
                      <div className="flex gap-4">
                        <Button
                          disabled={isPendingHire}
                          onClick={() =>
                            handleAcceptDeclineHire("accepted", hire.id)
                          }
                          className="bg-primary text-white w-fit text-xs rounded-[14px]"
                        >
                          {isPendingHire ? "Accepting..." : "Accept"}
                        </Button>
                        <Button
                          disabled={isPendingHire}
                          variant={"destructive"}
                          onClick={() =>
                            handleAcceptDeclineHire("declined", hire.id)
                          }
      ...
      <Dialog open={showDeclineModal} onOpenChange={setShowDeclineModal}>
        <DialogContent className="!rounded-3xl">
          ...
          <div className="flex flex-col gap-4 mt-2">
            <span className="text-sm text-[#878A93]">
              Are you sure you want to decline this opportunity? This action
              cannot be undone.
            </span>
